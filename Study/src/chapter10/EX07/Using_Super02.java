...
// 자식 클래스는 생성자 앞에 this(), super() 가 반드시 와야 한다
// 생략시 super(), 부모 클래스의 기본 생성자가 생략되어 있다
...
class Cc{	// 부모 클래스 
	// 객체 내부의 생성자가 존재할 경우, 컴파일러는 기본 생성자를 자동으로 추가하지 않는다 
	Cc () {}	// <= 이걸 넣으면 오류발생 안함
...
class Dd extends Cc {	// 자식 클래스
	...
	/* Dd 클래스의 생성자가 존재하기 않을때 컴파일러는 기본 생성자를 넣습니다 
	 	D() {
	 		super();	// 기본적으로 생략되어 있다 
	 	}	*/
	...
	Dd() {
 		super(3);
 		System.out.println("Dd의 기본 생성자 출력");
 	}
	
	Dd(int a){
		this();	// 자신 객체의 생성자 호출 
		System.out.println("Dd의 매개변수 1개인 생성자");
	}
}
